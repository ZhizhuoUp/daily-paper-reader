...
conclusion: 该研究证明了基于灵敏度的鲁棒控制策略能有效提升协作空中机器人在参数扰动下的安全性与可靠性。
---

## 摘要
本文提出了一种针对有界参数不确定性下协作式空中链的基于灵敏度的管状非线性模型预测控制（NMPC）框架。我们考虑了一个由刚性连杆连接的平面双飞行器链，并采用输入速率驱动建模，以强制执行推力和扭矩的旋转速率及幅值限制。通过在预测时域内传播一阶参数状态灵敏度，并利用其计算在线约束收紧裕度，实现了对连杆质量、长度和惯性不确定性的鲁棒性。我们对通过平滑余弦嵌入实现的连杆间分离约束以及推力幅值边界进行了鲁棒化处理。该方法在 MATLAB 中实现，并通过贴边机动和蒙特卡洛不确定性采样进行了评估。结果表明，在存在不确定性的情况下，该方法提高了约束裕度，且跟踪性能与标称 NMPC 相当。
