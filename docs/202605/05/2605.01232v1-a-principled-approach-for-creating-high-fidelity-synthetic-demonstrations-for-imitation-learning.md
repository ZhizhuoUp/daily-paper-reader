...
## 摘要
最近 3D 高斯泼溅 (3DGS) 的进展使得能够从单条专家轨迹和简短的多视图扫描中生成视觉上逼真的演示。然而，现有的基于 3DGS 的合成流水线通常使用基于采样的规划器或轨迹优化来生成新动作，这往往会大幅偏离专家的演示路径。虽然此类偏差对于对动作形状不敏感的任务可能是可以接受的，但它们丢弃了对于富接触和形状敏感的操作至关重要的细微时空结构，导致增加的演示多样性反而损害了下游策略学习。我们认为演示合成应该将专家轨迹视为一种强先验。基于这一原则，我们提出了一个框架，在合成多样化任务演示的同时，显式地保留专家运动结构。我们使用动态运动基元 (DMPs) 对专家轨迹进行建模，并将其重定向到重建的 3DGS 场景中的新目标、物体配置和视角，从而在构造上产生相位一致且保持形状的运动。为了在杂乱场景中安全地实现这种保留专家的多样性，我们引入了一种解析式障碍物感知 DMP 公式，该公式直接作用于由 3DGS 表示诱导的连续密度场。这实现了避障，同时对名义专家运动的扰动最小，在无需额外场景表示的情况下统一了照片级渲染和几何推理。我们在 Spot 移动操作机器人上评估了我们的方法，涵盖了三个对轨迹保真度敏感度递增的操作任务。与基于规划器和优化的合成相比，我们的方法产生的轨迹具有更低的偏差和碰撞率，并在训练基于扩散的视觉运动策略时获得了更高的任务成功率。
