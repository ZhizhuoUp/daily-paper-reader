...
## 摘要
离线安全强化学习 (RL) 旨在在严格的安全约束下，从静态数据集中寻求奖励最大化的策略。现有方法通常依赖于软期望成本目标或迭代生成式推理，这对于安全关键型实时控制可能不足。我们提出了 Safe Flow Q-Learning (SafeFQL)，通过将受 Hamilton-Jacobi 可达性启发的安全价值函数与高效的一步流策略相结合，将 FQL 扩展到安全离线强化学习。SafeFQL 通过自一致性 Bellman 递归学习安全价值，通过行为克隆训练流策略，并将其蒸馏为一步 Actor，以便在部署时无需拒绝采样即可进行奖励最大化的安全动作选择。为了考虑学习到的安全边界中的有限数据近似误差，我们增加了一个符合预测 (conformal prediction) 校准步骤，用于调整安全阈值并提供有限样本概率安全覆盖。实验表明，与扩散式安全生成基准相比，SafeFQL 以略高的离线训练成本换取了显著降低的推理延迟，这有利于实时安全关键型部署。在小船导航和 Safety Gymnasium MuJoCo 任务中，SafeFQL 达到或超过了先前的离线安全强化学习性能，同时大幅减少了约束违反。
