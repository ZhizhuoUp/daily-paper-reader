...
## 摘要
开放世界物体操控仍然是机器人学中的一个基本挑战。虽然视觉-语言-动作（VLA）模型已展示出令人期待的结果，但它们严重依赖大规模机器人动作演示，这些演示的收集成本高昂，且可能阻碍分布外泛化。在本文中，我们提出了一种基于显式世界模型的开放世界操控框架，通过构建环境的物理接地数字孪生来实现零样本泛化。该框架集成了开放集感知、数字孪生重建以及交互策略的采样与评估。通过构建环境的数字孪生，我们的方法能够在支持物理特性的模拟器中高效地探索和评估操控策略，并将选定的策略可靠地部署到现实世界中。实验表明，所提出的框架能够在没有任何特定任务动作演示的情况下执行多个开放集操控任务，证明了在任务和物体层面上强大的零样本泛化能力。项目页面：https://bojack-bj.github.io/projects/thesis/
