...
tldr: 针对人形机器人紧急停机可能导致摔倒的问题，本文提出了一种策略相关的安全可停机性概念。通过开发PRISM框架，利用重要性采样和仿真数据训练神经预测器，以识别机器人能够安全执行回退控制器的状态空间。该方法提高了数据效率，减少了误判，并成功在真实人形机器人上验证了其主动安全监控能力。
selection_source: fresh_fetch
motivation: 传统紧急停机对人形机器人可能造成灾难性后果，因此需要确定机器人能够安全执行平衡保持回退控制器的状态范围。
method: 提出了PRISM框架，通过重要性采样迭代优化决策边界，并利用仿真数据训练神经网络来预测状态级别的可停机性。
result: 实验表明PRISM显著提升了数据效率，降低了安全误报率，并实现了从仿真到真实人形机器人平台的成功迁移。
conclusion: 将安全性建模为策略相关的可停机性，为人形机器人提供了主动的安全监控手段和可扩展的失效保护行为认证。
---

## 摘要
紧急停止（E-stop）机制是机器人安全的行业标准。然而，对于人形机器人而言，突然切断电源本身可能会导致灾难性的故障；相反，紧急停止必须执行预定义的后备控制器，以保持平衡并将机器人引导至风险最小的状态。这提出了一个关键问题：人形机器人可以从哪些状态安全地执行此类停止？在这项工作中，我们将人形机器人的紧急停止形式化为一个依赖于策略的安全可停止性问题，并使用数据驱动的方法来表征安全可停止包络面。我们引入了 PRISM（基于重要性采样可停止性监测器的主动细化），这是一个仿真驱动的框架，用于学习状态级可停止性的神经预测器。PRISM 使用重要性采样迭代地细化决策边界，从而实现对稀有但安全关键状态的有针对性探索。这种有针对性的探索在固定仿真预算下显著提高了数据效率，同时减少了误报安全（false-safe）的预测。我们通过在真实人形平台上部署预训练的监测器，进一步演示了从仿真到现实（sim-to-real）的迁移。结果表明，将安全性建模为依赖于策略的可停止性，可以实现主动安全监测，并支持人形机器人故障安全行为的可扩展认证。
