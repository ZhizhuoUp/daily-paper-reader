...
## 摘要
生物感觉系统具有固有的适应性，能够过滤掉恒定刺激并优先处理相对变化，这可能提高了计算和代谢效率。受多种动物主动感知行为的启发，本文提出了一种新型的地面机器人基于事件的视觉伺服框架。利用动态视觉传感器（DVS），我们证明了通过对结构化对数光强变化模式产生的异步事件流应用固定空间核，所得的净事件通量可以解析地分离出特定的运动状态。我们为该事件速率估计器建立了一个广义理论界限，并表明线性和二次空间轮廓分别分离了机器人的速度以及位置-速度乘积。利用这些特性，我们采用多模式刺激来直接合成非线性状态反馈项，完全无需传统的状态估计。为了克服事件感知中固有的平衡点处线性可观测性不可避免的丧失，我们提出了一种仿生主动感知极限环控制器。在 1/10 比例自主地面车辆上的实验验证确认了所提出的直接感知方法的有效性、极低延迟和计算效率。
