...
## 摘要
高接触操作任务（如擦拭和组装）需要对接触力、摩擦力变化和状态转换进行准确感知，而这些信息无法仅凭视觉可靠地推断。尽管人们对视触觉操作的兴趣日益增长，但其进展受到两个持久局限性的制约：现有数据集规模较小且任务覆盖范围窄，且当前方法将触觉信号视为被动观测，而非利用它们来建模接触动力学或显式实现闭环控制。在本文中，我们提出了 OmniViTac，这是一个大规模的视触觉-动作数据集，包含跨越 86 个任务和 100 多个物体的 21,000 多条轨迹，并被组织成六种基于物理的交互模式。基于该数据集，我们提出了 OmniVTA，这是一个基于世界模型的视触觉操作框架，集成了四个紧密耦合的模块：一个自监督触觉编码器、一个用于预测短时域接触演化的双流视触觉世界模型、一个用于动作生成的接触感知融合策略，以及一个以 60Hz 频率运行的反射式控制器，用于在闭环中修正预测触觉信号与观测触觉信号之间的偏差。在所有六个交互类别上的真实机器人实验表明，OmniVTA 优于现有方法，并能很好地泛化到未见过的物体和几何构型，证实了将预测性接触建模与高频触觉反馈相结合在处理高接触操作中的价值。所有数据、模型和代码都将在项目网站 https://mrsecant.github.io/OmniVTA 上公开。
