...
tldr: 针对文本驱动的人体运动生成在机器人控制中存在的物理失真问题，本文提出了 PhysMoDPO 框架。该方法将全身控制器（WBC）集成到训练流程中，利用直接偏好优化（DPO）和物理/任务奖励微调扩散模型，使生成的运动在经过 WBC 处理后既符合物理规律又保持原始指令意图。实验证明该方法在模拟和真实 G1 人形机器人上均显著提升了运动真实感和任务成功率。
