...
tldr: "针对多机器人系统在包含不可控动态代理（如行人）的共享环境中面临的冲突与延迟问题，本文提出了流感知多智能体路径规划（FA-MAPF）框架。该框架通过将学习到的不可控代理运动模式集成到中心化MAPF算法中，有效利用环境信息优化路径。实验证明，FA-MAPF在不降低任务效率的前提下，能将与不可控代理的冲突减少高达55%，显著提升了机器人在复杂动态环境中的运行质量。"
selection_source: fresh_fetch
motivation: 现有的多智能体路径规划研究多关注延迟下的完备性，而忽视了利用环境中的动态代理运动模式来减少冲突。
method: 提出FA-MAPF框架，将不可控代理的预测运动流模式整合进中心化路径规划算法中。
result: "在仿真和真实人类轨迹数据集上的实验显示，该方法在保持效率的同时，最高可减少55%的冲突。"
conclusion: 通过感知并利用环境中的动态流信息，可以显著提升多机器人系统在共享空间中的安全性与协调性。
---
