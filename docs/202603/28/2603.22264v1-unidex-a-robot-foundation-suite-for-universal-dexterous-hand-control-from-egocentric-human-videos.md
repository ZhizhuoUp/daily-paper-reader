...
## 摘要
由于真实机器人遥操作数据采集成本高、手部构型异构性以及控制维度高等问题，灵巧操作仍然具有挑战性。我们提出了 UniDex，这是一个机器人基础套件，它将大规模以机器人为中心的数据集与统一的视觉-语言-动作（VLA）策略以及实用的真人数据采集方案相结合，用于通用灵巧手控制。首先，我们构建了 UniDex-Dataset，这是一个包含超过 5 万条轨迹、涵盖 8 种灵巧手（6-24 自由度）的以机器人为中心的数据集，该数据集源自第一视角人类视频数据集。为了将人类数据转化为机器人可执行的轨迹，我们采用了一种“人在回路”的重定向程序，在保持合理的手-物接触的同时对齐指尖轨迹，并对掩蔽了人手的显式 3D 点云进行操作，以缩小运动学和视觉差距。其次，我们引入了功能-执行器对齐空间（FAAS），这是一种统一的动作空间，将功能相似的执行器映射到共享坐标，从而实现跨手迁移。利用 FAAS 作为动作参数化方式，我们训练了 UniDex-VLA，这是一种在 UniDex-Dataset 上预训练并使用任务演示进行微调的 3D VLA 策略。此外，我们构建了 UniDex-Cap，这是一种简单的便携式采集装置，可记录同步的 RGB-D 流和人手姿态，并将其转换为机器人可执行的轨迹，以实现人机数据协同训练，减少对昂贵机器人演示的依赖。在两种不同灵巧手的挑战性工具使用任务中，UniDex-VLA 实现了 81% 的平均任务进度，大幅超越了之前的 VLA 基准模型，同时表现出强大的空间、物体和零样本跨手泛化能力。UniDex-Dataset、UniDex-VLA 和 UniDex-Cap 共同为通用灵巧操作提供了一个可扩展的基础套件。
