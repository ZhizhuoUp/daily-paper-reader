...
## 摘要
在危险环境中协调智能体（例如在冲突地区导航的救援无人机或在充满障碍的部署区域穿行的野外机器人）带来了基础性的规划挑战。我们提出并分析了一个捕捉该场景的新型多智能体路径规划问题的计算复杂度。一组相同的智能体从共同的起点出发，必须在基于图的环境中导航以到达共同的目标点。图中包含危险因素，这些因素在接触时会消除智能体，但在重新激活前会进入一个已知的冷却期。在这种离散时间、全观测、确定性的设定下，规划任务是计算一个移动调度方案，以最大化到达目标点的智能体数量。我们首先证明，尽管可行方案的空间呈指数级增长，但最优方案仅需要多项式级步数，从而确立了其属于 NP 类问题。随后我们证明，即使环境图是一棵树，该问题也是 NP 难的。在积极的一面，我们针对由起点到目标点的顶点不相交路径组成的图，提出了一种多项式时间算法。我们的研究结果为该问题建立了一个丰富的计算图景，识别出了难解和可解的部分。
