...
title_zh: ManiDreams：通过不确定性感知且任务特定的直觉物理实现鲁棒物体操作的开源库
...
tldr: 针对现实世界机器人操作中固有的不确定性，本文提出了开源框架 ManiDreams。该框架通过模块化抽象实现了分布状态表示、动力学预测和约束规范，将不确定性的表示与传播集成到规划循环中。通过“采样-预测-约束”循环增强基础策略的鲁棒性，在感知、参数和结构不确定性下表现优异，显著提升了机器人在复杂任务中的操作稳定性。
selection_source: fresh_fetch
motivation: 现有的动力学模型往往只关注预测误差，而忽略了现实世界操作中不可避免的不确定性对鲁棒性的影响。
method: 提出了一个模块化框架，通过“采样-预测-约束”循环将不确定性感知集成到规划中，支持多种动力学后端和约束规范。
result: 在 ManiSkill 任务和真实世界部署中，该框架在各种扰动下均保持了比强化学习基准更高的鲁棒性。
conclusion: ManiDreams 为实现鲁棒的机器人操作提供了一个灵活且可扩展的开源工具库，证明了在规划中显式处理不确定性的重要性。
---

## 摘要
动力学模型，无论是模拟器还是学习到的世界模型，长期以来一直是机器人操作的核心，但大多数模型侧重于最小化预测误差，而非应对一个更根本的挑战：现实世界的操作本质上具有不确定性。我们认为，不确定性下的鲁棒操作从根本上是一个集成问题：不确定性必须在规划循环中进行表示、传播和约束，而不仅仅是在训练期间被抑制。我们提出并开源了 ManiDreams，这是一个基于直觉物理模型进行不确定性感知操作规划的模块化框架。它通过可组合的抽象实现了这种集成，包括分布状态表示、后端无关的动力学预测以及用于动作优化的声明式约束规范。该框架明确解决了三种不确定性来源：感知、参数和结构。它通过一个“采样-预测-约束”循环封装任何基础策略，该循环根据分布结果评估候选动作，从而在无需重新训练的情况下增强鲁棒性。在 ManiSkill 任务上的实验表明，在强化学习（RL）基准性能显著下降的各种扰动下，ManiDreams 仍能保持鲁棒的性能。在推、取、接以及现实世界部署中的可运行示例展示了其在不同策略、优化器、物理后端和执行器之间的灵活性。该框架已在 https://github.com/Rice-RobotPI-Lab/ManiDreams 公开。
