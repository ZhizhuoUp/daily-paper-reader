...
title_zh: GRAFT：针对细粒度机器人操作的定位式且高效的在线强化自适应
...
## 摘要
预训练的视觉-语言-动作（VLA）策略为机器人操作提供了强大的先验，但将其在线自适应到细粒度的生物医学任务中仍然具有挑战性。任务的成功通常取决于细微的、依赖视角的视觉线索，而任务级奖励对于哪些区域重要提供的指导很少，这使得从有限的真实机器人交互中学习任务相关的视觉定位（visual grounding）变得困难。在线自适应还受到 VLA 推理和基于回放的更新所带来的计算成本的限制
