...
date: 2026-09-11
pdf: "https://arxiv.org/pdf/2609.12959v2"
tags: ["query:q1"]
score: 7.0
evidence: 用于导航的安全感知避障模块
tldr: 本文提出了一种基于高斯随机场（GRF）的分布式随机最优控制框架，用于解决模式导向群体机器人在几何控制、自组织及动态环境避障中的挑战。该方法将群体协作建模为贝叶斯推理任务，通过密度场编码几何模式，并结合无迹变换实现不确定性感知下的安全导航，实现了异构平台间的通用性与鲁棒性。
selection_source: fresh_fetch
motivation: 旨在解决群体机器人在复杂动态环境中实现几何模式控制、自组织及安全导航的综合性难题。
method: 通过将GRF扩展至时域，将群体协作转化为贝叶斯推理问题，并利用密度场进行模式编码及无迹变换进行避障。
result: 在蒙特卡洛仿真及多旋翼/固定翼无人机实机实验中，验证了该框架在异构平台下的通用性、自愈能力及安全性。
conclusion: 该框架为分布式群体机器人提供了一种统一、鲁棒且具备环境不确定性感知能力的随机最优控制解决方案。
---

## 摘要
尽管模式导向集群在多种应用中展现出巨大潜力，但它们在几何控制、自组织以及动态环境下的安全导航方面仍面临多重挑战。本文提出了一种基于高斯随机场（GRF）的随机最优控制框架，旨在统一的概率架构内解决这些问题。通过将 GRF 扩展至时间域，该框架将群体协同建模为贝叶斯推理任务，使集群能够适应环境不确定性、满足非凸约束，并协调不同平台间的异构动力学。我们开发了一个具备不确定性感知与安全意识的避障模块，用于处理随机障碍物运动下的导航问题。该模块采用无迹变换（Unscented Transform）来传播动态障碍物及邻近智能体的状态不确定性，从而为避障提供有原则的置信区间。此外，本文引入了密度引导的模式控制，将几何模式编码为隐式密度场。这种表示方法将模式规范与显式的智能体-目标分配解耦，从而以分布式方式促进了内在的自愈与弹性重构。我们通过多种场景下的蒙特卡洛模拟对所提框架进行了广泛评估。其模型无关的特性在四旋翼和固定翼无人机集群上得到了验证，突显了其在具有异构动力学的平台间的通用性。最后，通过 15 架四旋翼集群的室内实验以及 4 架定制自主四旋翼的室外部署，验证了该方法的有效性与鲁棒性。这些实验证实了所提框架在真实环境中维持可靠的几何模式转换及安全感知导航的能力。
