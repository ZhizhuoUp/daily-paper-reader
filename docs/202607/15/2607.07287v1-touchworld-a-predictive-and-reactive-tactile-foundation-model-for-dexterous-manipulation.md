...
tldr: TouchWorld是一个针对灵巧操作的预测性与反应性触觉基础模型。它解决了现有视觉-语言-动作模型在处理力、滑动等隐性接触状态时频率低、适应性差的问题。通过分层架构，该模型结合了高层子任务规划、视觉-触觉目标动作生成以及高频触觉残差修正，实现了语义泛化与局部接触自适应的平衡。在多项复杂任务中，其成功率显著优于现有基线，尤其在应对人类干扰时表现出极强的鲁棒性。
selection_source: fresh_fetch
motivation: 现有的灵巧操作模型往往将触觉视为低频观测，难以同时兼顾宏观任务推理与微观接触反馈的快速响应。
method: 提出一种分层策略，包含高层规划层、视觉-触觉目标条件策略以及用于在线残差修正的高频触觉细化策略。
result: "在六项长程且接触密集的灵巧操作任务中，TouchWorld在常规和受扰环境下分别取得了65.0%和53.7%的成功率，大幅领先基线模型。"
conclusion: 将触觉作为预测参考和快速反馈信号，能有效提升灵巧机器人在复杂环境下的语义泛化能力和局部接触适应性。
---

## 摘要
日常环境中的灵巧操作既需要预判也需要反应：机器人必须预测接触应如何演变，同时快速纠正由打滑、对齐偏差、抓取不稳定或力不匹配引起的局部误差。视觉和语言提供了语义和几何引导，但它们无法可靠地揭示诸如力、打滑和接触稳定性等隐藏的接触状态。尽管触觉传感能够暴露这些物理线索，但大多数现有策略将触觉视为单体动作模型中的低频观测流，将缓慢的任务推理、动作生成和快速的接触反馈耦合在单个循环中。我们推出了 TouchWorld，这是一种用于灵巧操作的预测性与反应式触觉基础模型。TouchWorld 采用分层策略，将视觉-语言子任务规划、触觉世界模型预测、视觉-触觉目标条件动作生成以及高频触觉残差细化分离开来。高层规划层生成可执行的子任务并预测触觉子目标；视觉-触觉目标条件策略生成名义动作块；而触觉条件细化策略则利用最近的触觉和本体感受反馈进行在线残差修正。通过将触觉同时作为预测性接触参考和快速反馈信号，TouchWorld 在保留视觉-语言-动作策略语义泛化能力的同时，提高了局部接触自适应性。在六个长时程且接触丰富的灵巧操作任务中，TouchWorld 在标准设置下实现了 65.0% 的成功率，在人为干扰下实现了 53.7% 的成功率，分别比最强的基准模型高出 15.7 和 18.5 个百分点。
