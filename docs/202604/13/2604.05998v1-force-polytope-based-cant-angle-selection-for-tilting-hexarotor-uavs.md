...
## 摘要
从机动性角度来看，倾转多旋翼无人机的主要优势在于可行执行力矩（wrench）的动态可变性，这是物理交互任务的关键资产。因此，应优化倾斜角选择，以确保高性能，同时避免突变并保持实际可行性。在此背景下，本文针对执行交互任务的星形相互依赖倾斜六旋翼无人机，提出了一种轻量级控制框架。该方法利用离线计算的零力矩力多胞形查找表，为所需的控制力识别可行的倾斜角，并通过平衡效率与平滑性来选择最优角度。该框架集成了几何全位姿控制器，通过 MATLAB/Simulink 中的蒙特卡洛模拟进行了验证，并与基准策略进行了对比。结果表明，该方法显著缩短了计算时间，提高了位姿跟踪性能，并具有竞争力的驱动效率。最后，在 Simscape 中进行的完整墙壁巡检任务物理仿真进一步证实了所提策略在交互场景中的可行性。
