...
tldr: 针对机器人在未知且充满接触的环境中的作业挑战，本文提出了一种鲁棒自适应反步阻抗控制（RABIC）策略。该方法无需机器人动力学参数，通过反步法跟踪阻抗模型，并利用泰勒级数和自适应估计器处理系统不确定性与外部干扰。实验证明，该策略在保证轨迹跟踪和力监控的同时，比传统PD控制更安全，具有半全局实际有限时间稳定性。
selection_source: fresh_fetch
motivation: 旨在解决机器人在包含外部干扰和未建模动力学的复杂未知环境中，难以实现精确阻抗控制和安全交互的问题。
method: 提出一种基于反步法的自适应阻抗控制方案，结合泰勒级数动力学估计器和外部力上界自适应估计器，实现无需先验参数的控制。
result: 仿真和实物实验表明，该方法在移动操作臂和Franka机器人上均实现了优于PD控制的安全性能、轨迹跟踪和力监控能力。
conclusion: RABIC框架为耦合移动及固定串行操作臂的自适应与学习阻抗控制研究提供了坚实的理论与实践基础。
---

## 摘要
本文针对在接触丰富且不确定环境中运行的机器人，提出了一种鲁棒自适应反步阻抗控制（RABIC）策略。所提出的控制策略考虑了系统的完整耦合动力学，并明确考虑了包括外部扰动和未建模动力学在内的主要不确定性来源，同时在实现过程中不需要机器人的动力学参数。我们为内环提出了一种基于反步法的自适应阻抗控制方案，以跟踪参考阻抗模型。为了处理不确定性，我们采用基于泰勒级数的估计器来估计系统动力学，并采用自适应估计器来确定外力的上界。稳定性分析证明了整个系统的半全局实际有限时间稳定性。为了验证所提方法的有效性，本文进行了移动操作机仿真场景以及在真实 Franka Emika Panda 机器人上的实验评估。与 PD 控制相比，所提方法在确保轨迹跟踪和力监测的同时，表现出更高的安全性。总体而言，RABIC 框架为耦合移动式和固定式串行链路操作机的自适应及基于学习的阻抗控制的未来研究奠定了坚实基础。
