...
title_zh: 大语言模型控制机器人系统的执行前安全门与任务安全契约
...
tldr: 针对大语言模型（LLM）控制机器人时缺乏安全验证的问题，本文提出SafeGate架构。该架构结合神经符号方法，在执行前通过提取结构化安全属性并应用确定性决策门过滤不安全指令。同时引入“任务安全合约”，利用Z3求解器在执行过程中监控约束，防止不安全状态转换。实验证明该方法能显著降低缺陷指令的接受率，提升了LLM机器人系统的安全性。
selection_source: fresh_fetch
motivation: 现有的LLM机器人系统缺乏在指令转化为代码前的安全验证机制，且在执行过程中难以持续监控不安全的状态转换。
method: 提出SafeGate架构，通过提取符合ISO 13482标准的结构化属性进行预执行过滤，并结合任务安全合约与Z3求解器进行运行时约束检查。
result: 在230个基准任务和真实机器人实验中，SafeGate显著减少了对缺陷指令的误判，同时保持了对良性任务的高接受度。
conclusion: 预执行安全门与运行时安全合约的结合是保障LLM控制机器人系统安全性的关键。
---

## 摘要
大语言模型（LLMs）越来越多地被用于将任务指令转换为机器人可执行代码，然而，该流程缺乏验证门，无法在指令被翻译为机器人代码之前检测出不安全和有缺陷的指令。此外，在缺乏持续约束监控的情况下，即使最初看起来安全的指令在执行过程中也可能产生不安全的状态转移。在本研究中，我们引入了 SafeGate，这是一种神经符号安全架构，旨在防止不安全的自然语言任务指令进入机器人执行阶段。借鉴 ISO 13482 安全标准，SafeGate 从自然语言指令中提取结构化的安全相关属性，并应用确定性决策门来授权或拒绝执行。此外，我们引入了任务安全契约（Task Safety Contracts），将通过安全门的指令分解为不变式、守卫条件和中止条件，以防止执行过程中的不安全状态转移。我们进一步结合了 Z3 SMT 求解，以强制执行源自任务安全契约的约束检查。我们在 230 个基准任务、30 个 AI2-THOR 模拟场景以及真实世界机器人实验中，将 SafeGate 与现有的基于 LLM 的机器人安全框架和基准 LLM 进行了评估。结果表明，SafeGate 在保持对良性任务高接受率的同时，显著降低了对缺陷指令的接受率，证明了执行前安全门对于大语言模型控制的机器人系统的重要性。
