...
tldr: 本研究针对有限时域线性二次调节器（LQR）问题，提出利用深度算子网络（DeepONet）学习微分Riccati方程的解算子。该方法通过离线构建从时变系统参数到Riccati轨迹的映射，将在线计算负担从重复的数值积分转向快速的模型评估。研究不仅提供了关于误差传播和闭环稳定性的理论保证，还设计了适用于矩阵值问题的定制化架构。实验表明，该方法在保持高精度的同时，显著提升了计算速度，为实时最优控制提供了可靠的替代方案。
selection_source: fresh_fetch
motivation: 旨在解决有限时域LQR问题中，针对不同系统参数重复数值求解非线性微分Riccati方程所带来的巨大计算开销。
method: 采用DeepONet架构学习系统参数到Riccati解轨迹的算子映射，并引入渐进式学习策略以应对高维系统的扩展性挑战。
result: 数值实验证明该方法在多种LQR场景下均能实现高精度近似和强泛化能力，且在线评估速度远超传统数值求解器。
conclusion: 该框架为参数化和实时最优控制提供了一种兼具理论可靠性、计算高效性与良好扩展性的数据驱动替代方案。
---

## 摘要
我们提出了一个计算框架，旨在利用学习到的算子代理来替代有限时域线性二次型调节器（LQR）问题中微分 Riccati 方程的重复数值求解。我们不再为每个新的系统实例求解非线性矩阵值微分方程，而是离线构建一个关联解算子的近似，该算子将随时间变化的系统参数映射到 Riccati 轨迹。生成的模型能够对各种系统的近似最优反馈进行快速在线评估，从而将计算负担从重复的数值积分转移到一次性的学习阶段。从理论角度来看，我们为这种基于算子的近似建立了控制理论保证。具体而言，我们推导了量化算子近似误差如何传播到反馈性能、轨迹精度和代价次优性的界限，并证明了在足够精确的算子近似下，闭环系统的指数稳定性得以保持。这些结果为评估最优控制中数据驱动近似的可靠性提供了一个框架。在计算方面，我们针对矩阵值、随时间变化的问题设计了定制的 DeepONet 架构，并引入了一种渐进式学习策略以解决关于系统维度的可扩展性问题。在时不变和时变 LQR 问题上的数值实验表明，所提方法在各种系统配置中均实现了高精度和强泛化能力，同时与传统求解器相比提供了显著的计算加速。该方法为参数化和实时最优控制应用提供了一种有效且可扩展的替代方案。
