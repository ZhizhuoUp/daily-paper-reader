...
tldr: PHAROS是一个针对安全关键型实时系统的异构加速器（HA）设计框架。针对现有框架侧重吞吐量而忽视实时调度性的问题，PHAROS引入了抢占机制和支持FIFO/EDF策略的调度器设计。通过结合现代实时理论，该框架实现了以软实时调度性为导向的设计空间探索（DSE），并提供响应时间分析。实验证明，PHAROS在保证任务截止日期合规性方面优于传统方法，能为多样化任务集提供更多可行配置。
selection_source: fresh_fetch
motivation: 现有的异构加速器设计框架主要优化吞吐量，缺乏对安全关键应用中实时调度性和截止日期合规性的硬件支持与分析。
method: 提出了PHAROS框架，包含硬件抢占机制、FIFO/EDF调度器设计，以及一种以软实时调度性为约束和目标的自动化设计空间探索方法。
result: 与以吞吐量为导向的基准测试相比，PHAROS能发现更多满足实时要求的硬件配置，并显著提升了任务的实时性能。
conclusion: PHAROS通过将实时调度理论融入硬件设计流程，为构建具有截止日期保障的安全关键型嵌入式系统提供了有效方案。
---

## 摘要
空间分区的异构加速器（HA）因其性能和灵活性而越来越多地被嵌入式系统采用。然而，大多数现有的 HA 设计框架主要针对吞吐量或服务质量（QoS）指标进行优化。它们往往忽视了安全关键的实时性要求，包括对可预测执行的硬件支持、实时感知的设计空间探索（DSE）以及严谨的可调度性分析。这些要求在智能交通等安全关键应用中至关重要，因为可调度性保证直接影响系统安全性。为了填补这一空白，我们提出了 PHAROS，一个以实时性为核心的 HA 设计框架。PHAROS 为先进先出（FIFO）和最早截止日期优先（EDF）策略下的空间分区 HA 引入了抢占机制和调度器设计。利用现代实时理论，我们进一步开发了面向软实时（SRT）可调度性的 DSE，其目标和约束针对 SRT 可调度性进行了定制。通过对多种应用的全面建模、分析和评估，我们表明，与面向吞吐量的 DSE 基准相比，PHAROS 的 DSE 能为更广泛的任务集发现更多可行配置，同时提供更好的实时性能。我们还为所支持的调度算法提供了响应时间分析。
