...
tags: ["query:q6"]
score: 7.0
evidence: 机器人操作与接触密集型任务的数据采集
tldr: TAMEn是一个触觉感知的机器人操作引擎，旨在解决复杂接触任务中的数据采集难题。它通过跨形态的可穿戴接口适配多种夹具，并结合动捕与VR技术实现高精度及便携式数据采集。该系统引入了包含触觉预训练、双臂演示和闭环恢复数据的金字塔式数据方案，显著提升了策略的鲁棒性和任务成功率，并开源了相关硬件与数据集。
selection_source: fresh_fetch
motivation: 现有的手持式演示采集方法在处理复杂接触任务时，面临硬件适配性差、缺乏在线可行性检查以及缺少真实触觉反馈和恢复数据的问题。
method: 提出了一种跨形态可穿戴接口和双模态采集管线（动捕与VR），并构建了整合触觉预训练与人机协作恢复数据的金字塔式学习框架。
result: "实验表明该系统显著提高了演示的可重放性，并将多种双臂操作任务的成功率从34%大幅提升至75%。"
conclusion: TAMEn通过融合触觉感知与闭环数据采集，为复杂接触环境下的双臂机器人操作提供了一套高效、鲁棒且开源的解决方案。
---
