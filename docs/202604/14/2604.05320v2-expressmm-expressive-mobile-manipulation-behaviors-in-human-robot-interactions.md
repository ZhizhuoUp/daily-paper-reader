...
result: 实验结果显示，该框架显著提升了用户对机器人动作意图的理解，并使其在协作中表现出更高的有用性、安全性和可预测性。
conclusion: ExpressMM通过整合多模态大模型与底层控制，为实现更自然、透明且具备社交适应性的移动机器人交互提供了有效方案。
---

## 摘要
移动操作机器人越来越多地被部署在以人为中心的环境中执行任务。在完成这些任务的同时，它们还应该能够通过表达性机器人行为向周围的人传达其意图。先前关于表达性机器人行为的研究采用了预编程或基于示教学习（learning-from-demonstration）的表达性动作，以及由大语言模型生成的高层交互。现有的大多数方法尚未考虑用户在任务执行过程中可能中断、修改或重定向机器人动作的人机交互（HRI）场景。在本文中，我们开发了新颖的 ExpressMM 框架，该框架将基于视觉语言模型（用于感知和对话推理）的高层语言引导规划器与底层视觉-语言-动作（VLA）策略相结合，以在协作式 HRI 任务中生成表达性机器人行为。此外，ExpressMM 支持可中断的交互，以适应用户更新或重定向的指令。我们在移动操作机器人辅助人类进行协作装配的场景中演示了 ExpressMM，并对现场 HRI 演示进行了基于受众的评估。问卷结果显示，由 ExpressMM 赋能的表达性行为有助于观察者清晰地解读机器人的动作和意图，同时支持符合社交规范且易于理解的交互。参与者还反馈称，机器人在协作任务中非常有用，且在演示过程中表现得可预测且安全，从而增强了人们对机器人在协作任务中的实用性、安全性和可预测性的正面认知。
