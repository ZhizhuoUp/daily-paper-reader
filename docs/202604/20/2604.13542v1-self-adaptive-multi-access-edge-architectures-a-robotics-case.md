...
tldr: 针对计算密集型AI任务在机器人场景下的性能与能效挑战，本文提出一种自适应多接入边缘架构。通过集成基于MAPE-K循环的智能监控代理，系统能根据实时响应时间和功耗，在Kubernetes编排的异构边缘环境中动态决策资源扩展与任务卸载。该方案在人机共存环境下优化了人类行为预测算法的执行，显著提升了系统的服务质量与运行效率。
selection_source: fresh_fetch
motivation: 旨在解决AI驱动的机器人系统在边缘计算中面临的高处理成本、性能瓶颈以及能源效率低下的问题。
method: 构建了一个由Kubernetes编排的异构分布式边缘卸载系统，并利用MAPE-K闭环机制实现自适应的资源缩放与任务调度决策。
result: 实验表明，该自适应架构在服务质量（QoS）方面优于传统静态配置，有效平衡了响应延迟与系统功耗。
conclusion: 研究证明了自适应边缘架构在提升复杂AI任务处理效率及保障人机协作环境安全性方面的显著优势。
---

## 摘要
计算密集型 AI 任务的增长凸显了降低处理成本、提升性能和能源效率的必要性。这要求集成智能代理作为架构自适应监督者，负责在计算连续体（continuum）内进行基础设施的自适应扩展和高效的计算卸载。本文提出了一种针对人机混合环境高效计算系统的自适应方法。该计算任务涉及一种神经网络算法，利用传感器数据预测人类移动行为，从而增强移动机器人的主动路径规划并确保人类安全。为了优化神经网络处理，我们构建了一个由 Kubernetes 编排的、包含异构处理单元的分布式边缘卸载系统。通过监测响应时间和功耗，基于 MAPE-K 的自适应监督者能够在扩展和卸载方面做出明智决策。结果表明，与传统配置相比，该方法在服务质量方面有显著提升，证明了所提方法在 AI 驱动系统中的有效性。
