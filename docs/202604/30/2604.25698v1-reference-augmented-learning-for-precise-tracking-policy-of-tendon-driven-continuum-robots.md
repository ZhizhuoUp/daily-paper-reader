...
title_zh: 面向线驱动连续体机器人精确跟踪策略的参考增强学习
...
tldr: "针对腱驱动连续体机器人（TDCR）因非线性、路径依赖和非马尔可夫特性导致的控制难题，本文提出一种参考增强的离线学习框架。该方法利用可微RNN动力学代理模型作为梯度桥梁，通过多尺度参考分布增强（包括随机偏差、谐波扰动和随机游走）优化控制策略。实验表明，该策略显著提升了6自由度跟踪精度，位置误差较基准降低50.9%，且在稳定性和泛化性上优于传统雅可比方法。"
selection_source: fresh_fetch
motivation: 传统雅可比控制难以应对迟滞引起的振荡，而常规学习方法在处理分布外轨迹时泛化能力不足。
method: 利用可微RNN动力学代理模型作为梯度桥梁，通过多尺度随机扰动增强参考分布，使策略学习到多样的误差恢复机制。
result: "在三段式TDCR平台上的实验显示，该方法比非增强基准降低了50.9%的平均位置误差，且在精度和稳定性上均优于雅可比方法。"
conclusion: 参考增强学习框架有效提升了连续体机器人在复杂动力学环境下的跟踪精度和鲁棒性。
---

## 摘要
线驱动连续体机器人 (TDCRs) 由于其高度非线性、路径依赖的动力学以及非马尔可夫特性，带来了显著的控制挑战。传统的基于雅可比矩阵的控制器通常难以应对迟滞引起的振荡，而传统的基于学习的方法在面对分布外轨迹时泛化能力较差。本文提出了一种参考增强的离线学习框架，用于 TDCRs 的精确 6 自由度跟踪控制。通过利用基于可微 RNN 的动力学代理模型作为梯度桥梁，我们通过增强的参考分布来优化控制策略。这种多尺度增强方案结合了随机偏差、谐波扰动和随机游走，迫使策略在无需额外硬件交互的情况下内化多种跟踪误差恢复机制。在三段式 TDCR 平台上的实验结果表明，与未增强的基准方法相比，所提出的策略在平均位置误差上降低了 50.9%，并且在各种速度下的精度和稳定性方面均显著优于基于雅可比矩阵的方法。
