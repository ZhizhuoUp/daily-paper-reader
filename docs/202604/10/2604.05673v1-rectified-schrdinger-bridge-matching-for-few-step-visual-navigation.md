...
title_zh: 针对少步视觉导航的修正薛定谔桥匹配
authors: "Wuyang Luan, Junhui Li, Weiguang Zhao, Wenjian Zhang, Tieru Wu, Rui Ma"
date: 2026-04-07
pdf: "https://arxiv.org/pdf/2604.05673v1"
tags: ["query:q5"]
score: 7.0
evidence: 视觉导航的实时机器人控制
tldr: 针对具身智能视觉导航中生成式策略推理速度慢的问题，本文提出Rectified Schrödinger Bridge Matching (RSBM)框架。该方法通过引入熵正则化参数ε统一了Schrödinger Bridge与确定性最优传输的速率场结构。RSBM利用速率场结构不变性，在减少积分步数的同时保持了多模态分布覆盖，仅需3步积分即可实现高性能导航，显著提升了实时控制效率。
selection_source: fresh_fetch
motivation: 现有的扩散模型和Schrödinger Bridge策略因高方差随机传输导致推理步数多，难以满足机器人实时控制的低延迟需求。
method: 提出RSBM框架，利用速率场结构不变性，通过调节熵正则化参数ε来平衡路径直度和多模态覆盖，并结合条件先验缩短传输距离。
result: "在无需蒸馏或多阶段训练的情况下，RSBM仅需3步积分即可达到94%的余弦相似度和92%的导航成功率。"
conclusion: RSBM成功缩小了高保真生成式策略与具身智能实时性要求之间的差距，为高效视觉导航提供了新方案。
---

## 摘要
视觉导航是具身智能（Embodied AI）中的核心挑战，要求自主智能体将高维感官观测转化为连续的长程动作轨迹。虽然基于扩散模型和薛定谔桥（SB）的生成式策略能有效捕捉多模态动作分布，但由于高方差的随机传输，它们需要数十个积分步，这为实时机器人控制带来了关键障碍。我们提出了修正薛定谔桥匹配（RSBM），该框架利用了标准薛定谔桥（$\varepsilon=1$，最大熵传输）与确定性最优传输（$\varepsilon\to 0$，如条件流匹配）之间共享的速度场结构，并由单一熵正则化参数 $\varepsilon$ 控制。我们证明了两个关键结果：（1）条件速度场的函数形式在整个 $\varepsilon$ 谱范围内是不变的（速度结构不变性），使得单一网络能够服务于所有正则化强度；（2）减小 $\varepsilon$ 会线性降低条件速度方差，从而实现更稳定的粗步长 ODE 积分。RSBM 锚定于一个缩短传输距离的学习到的条件先验，在中间值的 $\varepsilon$ 下运行，以平衡多模态覆盖范围和路径直度。实验表明，虽然标准桥需要 $\geq 10$ 步才能收敛，但 RSBM 仅需 3 个积分步即可实现超过 94% 的余弦相似度和 92% 的成功率——且无需蒸馏或多阶段训练——显著缩小了高保真生成策略与具身智能低延迟需求之间的差距。
