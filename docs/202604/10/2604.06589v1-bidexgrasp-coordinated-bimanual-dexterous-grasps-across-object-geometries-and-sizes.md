...
motivation: 双手机灵抓取因高维动作空间和缺乏大规模高质量标注数据，限制了其在复杂物体操作中的应用。
method: 提出了一种结合区域初始化与解耦力闭合优化的两阶段数据合成管线，并构建了具备双手机调模块和几何尺寸自适应策略的生成模型。
result: 构建了包含6351个物体和970万条数据的BiDexGrasp数据集，并在仿真和真实世界测试中验证了生成模型对未知物体的高质量抓取能力。
conclusion: 本研究通过大规模数据集和创新的生成框架，显著提升了双手机灵抓取的协调性与泛化能力，为复杂机器人操作奠定了基础。
---

## 摘要
双手动灵巧抓取是机器人领域中一个基础且极具前景的方向，但其进展受限于缺乏全面的数据集和强大的生成模型。在这项工作中，我们提出了 BiDexGrasp，它包含一个大规模双手动灵巧抓取数据集和一个新颖的生成模型。在数据集方面，我们提出了一种新颖的双手抓取合成流水线，以高效地标注物理可行的抓取数据用于数据集构建。该流水线通过一种两阶段合成策略来应对高维双手动抓取的挑战，包括高效的基于区域的抓取初始化和解耦的力封闭（force-closure）抓取优化。在该流水线的支持下，我们构建了一个大规模双手动灵巧抓取数据集，包含 6351 个尺寸在 30 到 80 厘米之间的多样化物体，以及 970 万条标注的抓取数据。基于该数据集，我们进一步引入了一个双手协同且几何尺寸自适应的灵巧抓取生成框架。该框架包含两个关键设计：一个双手协同模块和一种几何尺寸自适应的抓取生成策略，用于在未见物体上生成协同且高质量的抓取。在仿真和现实世界中进行的广泛实验证明了我们提出的数据合成流水线和学习到的生成框架的卓越性能。
