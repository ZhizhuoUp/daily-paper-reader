...
tags: ["query:q6"]
score: 6.0
evidence: 用于操作的实时轨迹学习和机器人柔顺性
tldr: 针对当前人机交互技能教学系统碎片化且缺乏通用安全性的问题，本文提出一种统一的多层控制框架。该框架通过三层结构实现：首先从单次演示中实时学习轨迹与变阻抗，提高效率；其次利用零空间优化提升动觉教学的直观性与避障能力；最后引入全身零空间顺应性确保交互安全。实验证明该系统在提升学习效率的同时，增强了机器人全身的交互安全性与直观性。
selection_source: fresh_fetch
motivation: 现有的机器人技能学习方法缺乏一个集高效性、直观性和全身安全性于一体的统一框架。
method: 提出一种包含实时变阻抗学习、零空间优化动觉教学以及全身零空间顺应性控制的三层架构。
result: 在7自由度库卡机器人上的实验表明，该框架能从单次不完美演示中高效获取技能，并实现安全的全身交互。
conclusion: 该统一框架为机器人技能获取提供了一种更安全、更直观且高效的解决方案，扩展了人机协作的应用范围。
---

## 摘要
当前用于技能教学的人机交互（HRI）系统较为零散，现有文献中的方法未能提供一个同时具备高效性、直观性和通用安全性的内聚框架。本文提出了一种新颖的分层控制框架，通过在通用机器人顺应性基础上实现鲁棒且顺应的示教学习（LfD），填补了这一基本空白。所提方法由三个递进且相互关联的阶段组成。首先，我们引入了一种实时 LfD 方法，能够从单次演示中同时学习轨迹和可变阻抗，显著提高了效率和复现保真度。为了确保高质量且直观的动觉教导（kinesthetic teaching），我们随后提出了一种零空间优化策略，该策略能主动管理奇异点，并在人类演示过程中提供一致的交互感。最后，为了确保通用安全性，我们引入了一种基础的零空间顺应方法，使机器人全身能够在不损害主任务性能的情况下，顺应性地适应学习后的外部交互。这一最终贡献将该系统转变为一个通用的 HRI 平台，超越了仅限于末端执行器（EE）的应用。我们通过在 7 自由度 KUKA LWR 机器人上进行的全面对比实验验证了整个框架。结果表明，该统一系统在广泛的人机协作任务中表现得更安全、更直观且更高效。
