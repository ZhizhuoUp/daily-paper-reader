...
title_zh: 使用收缩理论与区间分析学习经认证的神经网络控制器
...
tldr: 本研究提出一种联合训练神经网络控制器与黎曼度量的框架，通过形式化边界传播确保闭环系统的收缩性。针对传统对称矩阵不等式导致的过度保守性，该方法利用非对称矩阵函数及其区间包的顶点检查，在单次操作中验证整个区域的收缩性，解决了采样复杂度问题。该方法在10维无人机模型上实现了高效训练，能在10分钟内同时获得控制策略、收缩度量及验证的收缩区域。
selection_source: fresh_fetch
motivation: 传统的神经网络控制器验证方法在处理对称黎曼收缩不等式时存在过度保守性及采样复杂度过高的问题。
method: 通过分析非对称矩阵函数并利用GPU并行的区间包顶点检查，结合线性边界传播技术实现严谨的收缩性验证。
result: 在10维状态的四旋翼无人机模型上，该方法在10分钟内成功学习并验证了控制策略、收缩度量及收缩区域。
conclusion: 该框架为复杂动力系统提供了一种兼具计算效率与严谨理论保证的神经控制器学习与形式化验证方案。
---

## 摘要
我们提出了一种新颖的框架，利用形式化边界传播联合训练神经网络控制器和神经黎曼度量，并提供严格的闭环收缩保证。由于依赖性管理不佳，直接对对称黎曼收缩线性矩阵不等式进行边界限定会导致不必要的过度保守性。相反，我们分析了一个非对称矩阵函数 $G$，通过对其区间包（interval hull）进行 $2^n$ 个 GPU 并行化的顶点检查，即可一次性验证整个区间子集 $X$ 是否为收缩区域。这消除了以往基于 Lipschitz 的保证所面临的样本复杂度问题。此外，对于满足 Killing 场假设的控制仿射系统，我们的方法生成了一个显式跟踪控制器，仅需对学习到的策略进行两次前向推理，即可指数级稳定任何动力学可行的轨迹。利用 JAX 和 immrax 进行线性边界传播，我们将该方法应用于一个完整的 10 状态四旋翼模型。在不到 10 分钟的 JIT 后训练中，我们同时学习了控制策略 $π$、神经收缩度量 $Θ$ 以及一个经过验证的 10 维收缩区域 $X$。
