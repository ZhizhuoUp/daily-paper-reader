...
date: 2026-07-31
pdf: "https://arxiv.org/pdf/2607.29271v1"
tags: ["query:q6"]
score: 6.0
evidence: 针对接触密集型遥操作操作的阻抗重定向
...
## 摘要
固定的笛卡尔阻抗使得高接触遥操作演示变得实用，但确保任务进度和接触支撑的增益同时也决定了冲击力和力的变动性。我们研究了单次演示的控制器到控制器阻抗重定向问题。给定一个固定的笛卡尔阻抗指令序列 {K0, D0, xcmd}，任务流形分解阻抗重定向 (MDIR) 确定性地将记录的控制器重新参数化为可执行的任务通道变阻抗指令。MDIR 通过在演示轨迹附近保留投影的任务通道响应来解决这一局部重定向问题。它在控制链度量下，利用被动残差补集表示操作功、施力和支撑通道中的源响应，计算可执行的笛卡尔到流形重定向 (C2M) 基准，并应用流形约束参数优化 (MPO) 来选择一个具有更低腕部力峰值、冲量、力变动性和标称控制器功率的可行代表。在 Franka Panda 机器人上进行的平面擦拭、拾取与放置以及推取实验中，完整的 MDIR 控制器在所有 15 次闭环执行中均通过了任务检查，并且相对于固定阻抗演示，降低了所有四项侵略性指标。
