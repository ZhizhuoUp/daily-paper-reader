...
tldr: 本研究针对地月空间复杂的非线性环境，将航天器追逃问题建模为圆型限制性三体问题下的零和微分博弈。通过引入推力和参考轨道相位控制，实现了在周期及拟周期轨道上的灵活机动。研究采用带约束的离散时间微分动态编程算法，并利用共享时间正则化优化计算效率。结果证明了相位控制在提升机动性的同时能有效限制轨道偏离，强调了轨道几何结构在防御性任务设计中的核心作用。
selection_source: fresh_fetch
motivation: 旨在解决地月空间不稳定环境下，航天器防御性机动规划困难的问题。
method: 提出一种基于离散时间微分动态编程的零和微分博弈求解方法，并结合相位控制与时间正则化技术。
result: 实验表明相位控制增强了机动灵活性，且离散时间方法相比连续时间方法具有显著的计算优势。
conclusion: 参考轨道的几何形状对地月空间任务的逃逸机会和交会敏感性有重要影响，是任务设计的关键。
---

## 摘要
地月空间航天器运行在非线性和不稳定的环境中，这使得防御性机动规划变得困难。我们将地月空间航天器的追逃问题建模为圆型限制性三体问题中的零和微分博弈。每个航天器控制其推力和参考轨道相位，使其能够在保持在参考轨道附近的同时，沿着周期轨道运动并跨越拟周期环面。我们使用一种约束离散时间微分动态规划方法来求解该博弈，该方法强制执行硬输入约束。我们提出了一种共享时间正则化方法，在同步两个航天器的同时，细化近月点附近的离散化。在周期和拟周期轨道上的数值结果表明，相位控制在限制偏离参考轨道的同时提高了机动灵活性。与连续时间建模相比，离散时间方法还具有显著的计算优势。拟Halo轨道与拟近直线Halo轨道（NRHO）的对比表明，近月点提供了更大的逃逸机会，但也增加了交会的敏感性。这些结果表明，参考轨道的几何形状是地月空间防御任务设计的重要组成部分。
