...
## 速览
**TLDR**：本研究针对任务与运动规划（TAMP）中手动编写算子困难及学习算子在长序列任务中搜索效率低的问题，提出了一种自动生成宏算子和谓词选择的系统。该系统通过从演示数据中识别具有因果关联的动作序列并将其压缩为单个宏算子，同时剔除冗余谓词以缩小符号状态空间。实验表明，该方法在多个领域显著提升了规划速度，并能解决基准方法无法处理的长序列任务，有效增强了TAMP系统的实用性。 \
**Motivation**：现有的TAMP算子学习方法因缺乏对多步动作结构的捕捉以及冗余谓词的评估，导致在处理长序列任务时搜索效率低下甚至不可行。 \
**Method**：系统通过识别训练数据中因果关联的动作对生成宏算子，并自动剪枝掉所有未在学习算子中引用的谓词以优化符号状态。 \
**Result**：与基准方法相比，该方法在四个TAMP领域中实现了高达4.6倍的规划加速，并成功解决了原本无法求解的长序列复杂任务。 \
**Conclusion**：宏算子的发现与谓词优化不仅能显著提升规划效率，在复杂长任务中更是决定任务可解性的关键技术手段。
