...
## 速览
**TLDR**：NeuralParker是一个针对不规则环境设计的强化学习自动泊车规划器。针对现有方法在长距离路径推理上的局限，它通过目标相对顶点表示法编码全局几何信息，并结合学习到的圆弧策略与终端集成机制，实现了在复杂边界环境下的高成功率、高质量路径规划，且在真实配送车辆上验证了其低计算成本和有效性。 \
**Motivation**：现有的自动泊车规划器多依赖局部观测，难以在长距离、不规则的边界环境中进行有效的路径推理。 \
**Method**：提出一种混合规划器，利用目标相对顶点表示全局障碍物，并结合学习的曲率-长度策略与基于三次埃尔米特连接的终端集成机制。 \
**Result**：在长距离路径选择基准测试中，NeuralParker在规划成功率和轨迹质量上均优于现有基准，并成功应用于真实车辆。 \
**Conclusion**：该研究证明了结合全局几何表征与混合策略的强化学习方法能有效解决复杂环境下的长距离泊车规划难题。
