...
## 速览
**TLDR**：MANGO-Grasp是一个针对跨具身灵巧抓取的各向异性交互框架。它将物体表示为面向几何的3D高斯基元，将机器人手表示为形态运动学描述符。通过在关键点与基元对上建立马氏场，该方法能精确捕捉局部几何特征并引导抓取优化。实验表明，该方法在多具身基准测试中表现优异，具备极强的零样本迁移能力，并在实物实验中达到86%的成功率。 \
**Motivation**：现有跨具身抓取方法在物体局部几何表征和机器人形态运动学编码方面存在不足，限制了其在异构机械手上的泛化能力。 \
**Method**：提出利用面向几何的3D高斯基元表示物体，并结合马氏场建立各向异性的交互预测与优化框架，以统一的超参数实现跨具身抓取。 \
**Result**：在CMAP和MultiGripperGrasp基准测试中显著超越现有基准，并在未见过的机器人手上实现了高达16.57%的零样本性能提升。 \
**Conclusion**：MANGO-Grasp通过精细的几何与运动学建模，为实现通用且稳定的跨具身灵巧抓取提供了一种高效的解决方案。
