...
tags: ["query:q6"]
score: 6.0
evidence: 用于接触密集型操作任务的动作分块技术
tldr: 针对机器人模仿学习中动作分块跨越阶段边界导致的结构性时间不匹配问题，本文提出了TRACT框架。该方法通过将动作分块分解为当前阶段和未来边界，并利用任务局部图约束阶段顺序，实现了时间路由的动作预测。此外，引入因果响应缺失积分器以优化接触密集型任务的执行。实验证明，TRACT在多项真实机器人任务中显著提升了成功率，消除了阶段歧义和停顿。
...
conclusion: 通过时间路由表示和时序权威性约束，TRACT有效解决了动作分块中的阶段转换问题，显著增强了机器人在复杂接触任务中的鲁棒性。
---
